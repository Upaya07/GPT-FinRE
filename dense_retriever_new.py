...
def encode_field(text, idx):
    # field_getter = kwargs['field_getter']
    # tokenizer = kwargs['tokenizer']
    # truncation = kwargs['truncation']
    # text = field_getter(example)
    tokenized_inputs = tokenizer.encode_plus(text, truncation=True, return_tensors='pt')
    return {
...
class DenseRetriever:
    ...
    def forward2(self, **kwargs):
        res_list = []
        open_file = open("test_kdf.json","r")
        for i, entry_dp in enumerate(open_file.readlines()[:5]):
            entry_dp = json.loads(entry_dp)
            # print("\n\n\n********start********\n\n\n")
            entry = encode_field(entry_dp["sentence"], i)
            entry["input_ids"] = entry["input_ids"].unsqueeze(0).to("cuda:0")
            entry["attention_mask"] = entry["attention_mask"].unsqueeze(0).to("cuda:0")
            # print(i,entry.keys())
            # print("\n\n\n********end********\n\n\n")
            with torch.no_grad():
                metadata = entry.pop("metadata")
    ...
    def find(self):
        ...
        res_list = self.forward2()
        all_result = []
        for res in res_list:
            # print("\n\n\n********end********\n\n\n")
            # print(res)
            # print(type(self.dataset_reader.dataset_wrapper[res['metadata']['id']]))
            # print("\n\n\n********end********\n\n\n")
            # res['entry'] = self.dataset_reader.dataset_wrapper[res['metadata']['id']]
            open_file = open("test_kdf.json","r")
            res['entry'] = json.loads(open_file.readlines()[0])
            set_global_object(self.index, self.is_train)
            # data = dpp(res, num_candidates=1, num_ice=5, mode="map", dpp_topk=100, scale_factor=0.1)
            data = knn(res, num_candidates=1, num_ice=10)
        ...
        # data = dpp(entry, num_candidates=1, num_ice=1, mode="map", dpp_topk=100, scale_factor=0.1)
